...
类模板的成员函数**不是在模板定义时创建**的，而是在**该成员函数被调用时才创建**（实例化）。
...
3. **注意**：这也意味着，**模板代码中的语法错误可能在不被调用时"隐藏"起来**，直到某次实例化时才暴露。
...
- **优点**：最通用，可以接受任何有 `showInfo()` 方法的类型（鸭子类型思想）。
...
### 6.3 返回类型中使用类模板（requires attention）
...
public:
    ...
    explicit MyArray(int capacity)
        : m_capacity(capacity)
        , m_size(0)
        , m_pAddress(new T[capacity]{})   // 值初始化
    {
        if (capacity <= 0) {
...
| 注意事项     | 说明                                                         |
| ------------ | ------------------------------------------------------------ |
| 默认构造函数 | 自定义类型必须提供默认构造函数（因为 new T[n] 会调用它）     |
